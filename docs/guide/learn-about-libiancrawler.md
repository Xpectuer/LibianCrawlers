...
| 原始数据 存储                           | 脚本爬虫通常写入 csv 和 sqlite                  | 在工程目录里乱拉屎，文件体积更是奇大也迫使 pandas/polars 不得不批次处理引发代码屎山危机。 原始数据的冗余经常由于各种原因被丢失。 | 在专用的 postgres **数据沼泽** 里拉屎。postgres 的高效 jsonb 格式也非常节省存储空间。                                                                                 |                                                                       
