...
| 清洗为 数据库对象                         | pandas/polars 将数据写入到新 csv 或某个数据库       | 难以管理数据库字段版本迁移。写入csv则又一次的在工程目录里乱拉屎。 pandas/polars 的无类型提示，也加重了转换的心智负担。     | 写入专用的 postgres **数据湖** 。其表和字段的版本迁移使用 Kysely Migration 进行管理，且同样使用 **typescript 上下游环节类型检查** 确保 数据对象 to 数据库新字段 的正确性。                          |                                              
